...
(defun ghostel--self-insert ()
  ...
  (let* ((keys (this-command-keys))
    ...
    ;; Send to the shell process
    (ghostel--send-key str)
    ;; Also feed directly to the terminal for immediate display.
    ;; The shell's line editor has stty -echo and does its own
    ;; redraw, but that output may arrive late or not at all
    ;; depending on PTY timing.  Local echo ensures the character
    ;; is visible immediately; the shell's subsequent redraw will
    ;; just confirm the same content.
    (when ghostel--term
      (ghostel--write-input ghostel--term str)
      (ghostel--invalidate))))
