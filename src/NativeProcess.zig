...
/// Platform-appropriate invalid fd sentinel for posix.fd_t.
/// On POSIX, fd_t is i32 so -1 works.  On Windows, fd_t is *anyopaque.
const invalid_fd: posix.fd_t = if (builtin.os.tag == .windows)
    @as(posix.fd_t, @ptrFromInt(std.math.maxInt(usize)))
else
    @as(posix.fd_t, -1);

process: PtyProcess,
event_pipe: posix.fd_t = invalid_fd,
...
wake_pipe: [2]posix.fd_t = .{ invalid_fd, invalid_fd },
quit: bool = false,
thread: std.Thread,
...
pub fn init(
    ...
    if (comptime builtin.os.tag == .windows) return error.NotSupported;
...
fn blockSigpipe(self: *Self) void {
    ...
    if (comptime builtin.os.tag != .windows and @hasDecl(posix.F, "SETNOSIGPIPE")) {
